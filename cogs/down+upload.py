...
def download_vid(link: str, folder_path: Path) -> Path:
    """
    download_vid.
    ...
    res = float(
        re.sub("[^0-9]", "", vid_stream.resolution)
    )  # The re.sub removes all characters that aren't numbers (Ex. 720p --> 720)

    kbitrate = float(
        re.sub("[^0-9]", "", aud_stream.abr)
    )  # The re.sub removes all characters that aren't numbers (Ex. 128kbps --> 128)
    bpf = (kbitrate * 1000) / fps

    print(f"Down*Upload: res: {res}, bpf: {bpf}")

    # Custom filename

    # Creates a filename in the format of "[youtube video title]_[date in ymdhms]_.mp4
    # Ex. Playing A Guitar_2022-12-26 21:20:35_.mp4"
    vid_filename = "_".join(["vid" + vid_stream.title + datetime.now().strftime("%Y-%m-%d %H-%M-%S") + ".mp4"])
    # Creates a filename in the format of "[youtube video title]_[date in ymdhms]_.wav Ex.
    # Playing A Guitar_2022-12-26 21:20:35_.wav"
    aud_filename = "_".join(["aud" + aud_stream.title + datetime.now().strftime("%Y-%m-%d %H-%M-%S") + ".wav"])
...
def upload_vid(
    ...
    login_storage_path : Pathlib.Path
            Path to the stored login information - this is used to store the login
            information without having to sign in again, also useful when you can't
            login from where this code will be run.
    Returns
    -------
    video_link : str
    ...
    print(f"Is base path: {base_path} used?")
    print(f"Is thumbnail path: {thumbnail_path} used?")
    channel = Channel()
